    ...
    <PublicLayout>
        <main class="bg-[#f7fbff] text-slate-950">
            <section class="border-b border-slate-200 bg-[linear-gradient(135deg,#f7fbff_0%,#eefbf8_48%,#fff7ed_100%)]">
                <div class="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 sm:py-14 lg:grid-cols-[minmax(0,1.08fr)_minmax(360px,0.92fr)] lg:px-8 lg:py-16">
                    <div class="flex flex-col justify-center">
                        <div class="mb-5 inline-flex w-fit items-center gap-2 rounded-lg border border-teal-100 bg-white/80 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-teal-700 shadow-sm">
                        ...
                        <h1 class="max-w-4xl text-[1.75rem] font-bold leading-[1.15] tracking-tight text-slate-950 sm:text-4xl lg:text-5xl">
                            Find a used car that feels right before you call
                        </h1>
                        <p class="mt-4 max-w-2xl text-sm font-normal leading-7 text-slate-600 sm:mt-5 sm:text-base sm:leading-8">
                        ...
                        <form class="mt-7 rounded-lg border border-slate-200 bg-white p-3 shadow-xl shadow-slate-200/70 sm:p-4" @submit.prevent="submitSearch">
                            <div class="grid gap-3 lg:grid-cols-[minmax(0,1.4fr)_minmax(160px,0.8fr)_minmax(160px,0.8fr)_auto]">
                                <label class="block">
                                    <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">Search model</span>
                                    <input
                                ...
                                <label class="block">
                                    <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">City</span>
                                    <select
                                ...
                                <label class="block">
                                    <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">Budget</span>
                                    <select
                                ...
                                <button
                                    type="submit"
                                    class="mt-1 inline-flex h-12 items-center justify-center gap-2 rounded-lg bg-slate-950 px-5 text-sm font-semibold text-white shadow-lg shadow-slate-300 transition hover:-translate-y-0.5 hover:bg-teal-700 lg:mt-6"
                                >
                        ...
                        <div class="mt-7 grid max-w-3xl gap-3 sm:grid-cols-3">
                            <div v-for="stat in stats" :key="stat.label" class="rounded-lg border border-white bg-white/75 p-3 shadow-sm sm:p-4">
                                <p class="text-xl font-semibold text-slate-950 sm:text-2xl">{{ stat.value }}</p>
                    ...
                    <div class="grid content-between gap-4">
                        <div class="rounded-lg border border-slate-200 bg-white p-3 shadow-2xl shadow-slate-200/80">
                            <div class="relative overflow-hidden rounded-lg bg-slate-100">
                                ...
                                <div class="flex flex-col gap-2 min-[420px]:flex-row min-[420px]:items-end min-[420px]:justify-between min-[420px]:gap-3">
                                        <div class="min-w-0">
                                            <p class="text-xs font-semibold uppercase tracking-wide text-teal-700">Fresh pick</p>
                                            <h2 class="mt-1 truncate text-lg font-semibold text-slate-950">{{ heroCar ? carTitle(heroCar) : 'Verified used cars' }}</h2>
                                            <p class="mt-1 text-sm font-semibold text-slate-500">{{ heroCar?.city || 'Across Bihar' }}</p>
                                        </div>
                                        <p class="shrink-0 text-lg font-bold text-orange-600">{{ heroCar ? formatPrice(heroCar.price) : 'Browse' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid gap-3 sm:grid-cols-2">
                            ...
                            >
                                <p class="text-sm font-semibold text-orange-700">Sell smarter</p>
                            ...
                            >
                                <p class="text-sm font-semibold text-teal-700">Find dealers</p>
            ...
            <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
                <div class="grid gap-4 md:grid-cols-3">
                    ...
                    >
                        ...
                        <span class="mt-4 inline-flex text-sm font-semibold text-teal-700 group-hover:text-orange-600">{{ item.action }}</span>
                    </component>
                </div>
            </section>
            ...
            <section class="bg-white">
                <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
                    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                        <div>
                            <p class="text-sm font-semibold uppercase tracking-wide text-orange-600">Curated inventory</p>
                            <h2 class="mt-2 text-3xl font-semibold text-slate-950 sm:text-4xl">Cars worth shortlisting today</h2>
                        ...
                        <Link href="/cars" class="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-900 transition hover:border-teal-600 hover:text-teal-700 sm:w-fit">
                            View all cars
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        >
                            <div class="relative aspect-[16/11] overflow-hidden bg-slate-100">
                                ...
                                <div class="absolute left-3 top-3 flex gap-2">
                                    <span v-if="car.is_verified" class="rounded-md bg-white px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-teal-700 shadow-sm">Verified</span>
                                    <span v-if="car.is_featured" class="rounded-md bg-orange-500 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-white shadow-sm">Featured</span>
                                </div>
                            </div>
                            <div class="p-4">
                                <h3 class="line-clamp-2 text-base font-semibold leading-snug text-slate-950 group-hover:text-teal-700">{{ carTitle(car) }}</h3>
                                <p class="mt-1 truncate text-sm font-semibold text-slate-500">{{ car.city || 'Bihar' }}</p>
                                <div class="mt-4 grid gap-2 text-center text-[11px] font-semibold text-slate-600 sm:grid-cols-3">
                                ...
                                <div class="mt-4 flex flex-col items-start gap-3 border-t border-slate-200 pt-4 sm:flex-row sm:items-end sm:justify-between">
                                    <div>
                                        <p class="text-xs font-medium uppercase tracking-wide text-slate-400">Asking price</p>
                                        <p class="mt-1 text-xl font-bold text-slate-950">{{ formatPrice(car.price) }}</p>
                                    </div>
                                    <span class="rounded-lg bg-teal-700 px-3 py-2 text-xs font-semibold text-white">Details</span>
                                </div>
                            </div>
                        </Link>
                    </div>
                    ...
                    <div v-else class="mt-8 rounded-lg border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
                        <h3 class="text-xl font-semibold text-slate-950">Inventory is being refreshed</h3>
                        <p class="mt-2 text-sm font-medium text-slate-600">Browse all cars or check again soon for fresh verified listings.</p>
                        <Link href="/cars" class="mt-5 inline-flex rounded-lg bg-slate-950 px-5 py-3 text-sm font-semibold text-white">Browse Cars</Link>
                    </div>
                </div>
            </section>
            ...
            <section class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
                <div class="max-w-2xl">
                    <p class="text-sm font-semibold uppercase tracking-wide text-teal-700">Quick discovery</p>
                    <h2 class="mt-2 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">Start with budget, brand or city</h2>
                ...
                <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div class="flex items-center gap-3">
                            <span class="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-teal-50 text-teal-700">
                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7H4a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2ZM16 7V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v2m14 6h.01" />
                                </svg>
                            </span>
                            <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-500">By budget</h3>
                        </div>
                        <div class="mt-4 flex flex-wrap gap-2">
                            <Link
                                ...
                                class="rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition hover:border-teal-300 hover:bg-teal-50 hover:text-teal-700"
                            >
                                {{ option.label }}
                            </Link>
                        </div>
                    ...
                    <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                        <div class="flex items-center gap-3">
                            <span class="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-teal-50 text-teal-700">
                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01" />
                                </svg>
                            </span>
                            <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-500">Popular brands</h3>
                        </div>
                        <div class="mt-4 flex flex-wrap gap-2">
                            <Link
                    ...
                    <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:col-span-2 lg:col-span-1">
                        <div class="flex items-center gap-3">
                            <span class="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-teal-50 text-teal-700">
                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a2 2 0 0 1-2.827 0l-4.244-4.243a8 8 0 1 1 11.314 0z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 1 1-6 0 3 3 0 0 1 6 0z" />
                                </svg>
                            </span>
                            <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-500">Nearby cities</h3>
                        </div>
                        <div class="mt-4 flex flex-wrap gap-2">
                            <Link
                ...
                <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
                    <div class="grid gap-6 lg:grid-cols-3">
                        <div class="lg:col-span-1">
                            <p class="text-sm font-semibold uppercase tracking-wide text-orange-600">Buying flow</p>
                            <h2 class="mt-2 text-2xl font-semibold text-slate-950 sm:text-3xl">A calmer path from search to shortlist</h2>
                        </div>
                        <div class="grid gap-4 md:grid-cols-3 lg:col-span-2">
                            <div v-for="step in steps" :key="step.title" class="rounded-lg border border-slate-200 bg-white p-5">
                                <p class="text-xs font-semibold uppercase tracking-wide text-teal-700">{{ step.number }}</p>
            ...
            <section class="mx-auto grid max-w-7xl gap-5 px-4 py-12 sm:px-6 lg:grid-cols-2 lg:px-8">
                <div class="rounded-lg border border-orange-100 bg-orange-50 p-6 sm:p-8">
                    <p class="text-sm font-semibold uppercase tracking-wide text-orange-700">For sellers</p>
                    <h2 class="mt-2 text-2xl font-semibold text-slate-950 sm:text-3xl">List your car without making it complicated</h2>
                    ...
                    <Link href="/sell-your-car" class="mt-6 inline-flex w-full justify-center rounded-lg bg-orange-500 px-5 py-3 text-sm font-semibold text-white transition hover:bg-orange-600 sm:w-fit">
                        Sell Your Car
                    </Link>
                </div>

                <div class="rounded-lg border border-teal-100 bg-teal-50 p-6 sm:p-8">
                    <p class="text-sm font-semibold uppercase tracking-wide text-teal-700">For dealers</p>
                    <h2 class="mt-2 text-2xl font-semibold text-slate-950 sm:text-3xl">Bring your inventory into a cleaner showroom</h2>
                    <p class="mt-3 text-sm font-medium leading-7 text-slate-700">
                        Verified dealer pages, catalog links and enquiry flow help your cars look more trustworthy online.
                    </p>
                    <Link href="/dealer/register" class="mt-6 inline-flex w-full justify-center rounded-lg bg-teal-700 px-5 py-3 text-sm font-semibold text-white transition hover:bg-teal-800 sm:w-fit">
                        Register Dealer
                    </Link>
                </div>
            </section>
            ...
            <section class="bg-white">
                <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
                    <div class="rounded-lg border border-slate-200 bg-[#f7fbff] p-6 sm:p-8">
                        <p class="text-sm font-semibold uppercase tracking-wide text-slate-500">Used cars in Bihar</p>
                        <h2 class="mt-2 text-2xl font-semibold text-slate-950">SahiGadi helps buyers compare second hand cars with more confidence</h2>
...
const withPlus = (count: number) => `${new Intl.NumberFormat('en-IN').format(count)}${count >= 10 ? '+' : ''}`;

const stats = computed(() => [
    { value: withPlus(allCars.value.length), label: 'cars listed' },
    { value: withPlus(topCities.value.length), label: 'cities covered' },
    { value: withPlus(topBrands.value.length), label: 'brands available' },
]);
