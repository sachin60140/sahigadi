                ...
                <template v-else>
                    <header :class="hasPending ? 'border-orange-200 bg-orange-50' : 'border-teal-100 bg-teal-50'" class="rounded-lg border p-5 sm:p-6">
                        <div class="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-xs font-semibold uppercase tracking-wide" :class="hasPending ? 'text-orange-700' : 'text-teal-700'">
                                        {{ hasPending ? 'Pending challans found' : 'No pending challans' }}
                                    </p>
                                    <h1 class="mt-1 text-2xl font-semibold uppercase text-slate-950 sm:text-3xl">{{ challanSearch.vehicle_number }}</h1>
                                </div>
                            </div>
                            <div class="grid grid-cols-2 gap-3">
                                <div class="rounded-lg border border-white/80 bg-white px-4 py-3 text-center shadow-sm">
                                    <p class="text-xs font-semibold uppercase text-slate-400">Challans</p>
                                    <p class="mt-1 text-xl font-semibold text-slate-950">{{ challanSearch.challan_count || 0 }}</p>
                                </div>
                                <div class="rounded-lg border border-white/80 bg-white px-4 py-3 text-center shadow-sm">
                                    <p class="text-xs font-semibold uppercase text-slate-400">Amount</p>
                                    <p class="mt-1 text-xl font-semibold text-orange-600">{{ money(challanSearch.total_amount) }}</p>
                                </div>
                            </div>
                        </div>
                    </header>
                    ...
                    <section v-if="challans.length" class="mt-6 rounded-lg border border-slate-200 bg-white shadow-sm">
                        <div class="border-b border-slate-200 px-5 py-4">
                            <p class="text-xs font-semibold uppercase tracking-wide text-orange-600">Traffic records</p>
                            <h2 class="mt-1 text-xl font-semibold text-slate-950">Challan details</h2>
                        </div>
                        ...
                        <div class="grid gap-4 p-4 md:hidden">
                            <article v-for="(challan, index) in challans" :key="challan.challanNo || index" class="rounded-lg border border-slate-200 p-4">
                                <div class="flex items-start justify-between gap-3">
                                    <div>
                                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Challan number</p>
                                        <p class="mt-1 text-sm font-semibold text-slate-950">{{ challan.challanNo || 'N/A' }}</p>
                                    </div>
                                    <span :class="challan.status === 'Paid' ? 'bg-teal-50 text-teal-700' : 'bg-red-50 text-red-700'" class="rounded-md px-2.5 py-1 text-xs font-semibold">
                                ...
                                <dl class="mt-4 grid gap-3">
                                    ...
                                    <div>
                                        <dt class="text-xs font-semibold uppercase tracking-wide text-slate-400">Offence</dt>
                                    ...
                                    <div>
                                        <dt class="text-xs font-semibold uppercase tracking-wide text-slate-400">Amount</dt>
                                        <dd class="mt-1 text-sm font-semibold text-orange-600">{{ money(challan.amountChallan) }}</dd>
                                    </div>
                                </dl>
                            </article>
                        </div>
                            ...
                            <table class="min-w-full text-left text-sm">
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    <tr v-for="(challan, index) in challans" :key="challan.challanNo || index">
                                        <td class="whitespace-nowrap px-4 py-4 font-semibold text-slate-900">{{ challan.challanNo || 'N/A' }}</td>
                                        ...
                                        <td class="whitespace-nowrap px-4 py-4 text-right font-semibold text-orange-600">{{ money(challan.amountChallan) }}</td>
                                        <td class="px-4 py-4">
                                            <span :class="challan.status === 'Paid' ? 'bg-teal-50 text-teal-700' : 'bg-red-50 text-red-700'" class="rounded-md px-2.5 py-1 text-xs font-semibold">
                                                {{ challan.status || 'Unknown' }}
                                            </span>
                    ...
                    <div v-else-if="challanSearch.is_success" class="mt-6 rounded-lg border border-teal-100 bg-white p-8 text-center shadow-sm">
                        ...
                        <h2 class="mt-3 text-xl font-semibold text-slate-950">No pending challans found</h2>
