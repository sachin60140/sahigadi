    ...
    <AdminLayout title="Dealer RC Searches" eyebrow="RC operations">
        ...
        <section class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wide text-teal-700">Dealer lookup ledger</p>
                    <h2 class="mt-2 text-3xl font-semibold text-slate-950">Track every dealer RC lookup and its result.</h2>
        ...
        <section class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <form class="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.15fr_1fr_160px_170px_170px_auto]" @submit.prevent="applyFilters">
                ...
                <div class="flex items-end gap-2 md:col-span-2 xl:col-span-1">
                    ...
                    <Link href="/admin/vehicle-searches" class="grid h-12 place-items-center rounded-lg border border-slate-200 px-5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Clear</Link>
                </div>
            </form>
        </section>
                ...
                <table class="w-full min-w-[1120px] text-left text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="search in searches.data" :key="search.id" class="hover:bg-slate-50">
                            ...
                            <td class="px-5 py-4">
                                <p class="font-semibold text-slate-950">{{ search.owner_name || 'Owner unavailable' }}</p>
                            ...
                            <td class="px-5 py-4">
                                <Link v-if="search.dealer" :href="search.dealer.show_url" class="font-semibold text-slate-950 hover:text-teal-700">{{ search.dealer.name }}</Link>
                        ...
                        <tr v-if="!searches.data.length">
                            <td colspan="7" class="px-5 py-14 text-center">
                                <p class="text-lg font-semibold text-slate-950">No dealer RC searches found</p>
