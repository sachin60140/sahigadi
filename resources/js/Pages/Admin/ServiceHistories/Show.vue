    ...
    <AdminLayout title="Service History Details" eyebrow="Service operations">
        ...
        <section class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div>
                    <Link :href="search.actions.back" class="inline-flex rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Back to reports</Link>
                    <p class="mt-5 text-xs font-semibold uppercase tracking-wide text-teal-700">Workshop timeline</p>
                    <h2 class="mt-2 text-3xl font-semibold uppercase text-slate-950">{{ search.vehicle_number }}</h2>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <a v-if="search.is_success" :href="search.actions.pdf" class="rounded-lg bg-slate-950 px-4 py-3 text-sm font-semibold text-white hover:bg-teal-700">Download PDF</a>
                </div>
            </div>
        </section>
        ...
        <section class="mt-5 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-5 py-4"><p class="text-xs font-semibold uppercase tracking-wide text-teal-700">Service records</p><h3 class="mt-1 text-xl font-semibold text-slate-950">{{ search.records.length }} workshop visits</h3></div>
            <div v-if="search.records.length" class="overflow-x-auto">
                <table class="w-full min-w-[1080px] text-left text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100"><tr v-for="(record,index) in search.records" :key="index"><td class="px-5 py-4 font-semibold text-slate-950">{{ record.date || 'N/A' }}</td><td class="px-5 py-4"><p class="font-semibold text-slate-950">{{ record.dealer_name || 'N/A' }}</p><p class="mt-1 text-xs font-semibold text-slate-500">{{ record.location_name || '' }}</p></td><td class="px-5 py-4"><p class="font-bold text-slate-700">{{ record.work_type || 'N/A' }}</p><p class="mt-1 text-xs font-semibold text-slate-500">{{ record.service_category || '' }}</p></td><td class="px-5 py-4 font-semibold text-slate-600">{{ record.repair_order_no || 'N/A' }} / {{ record.bill_no || 'N/A' }}</td><td class="px-5 py-4 font-semibold text-slate-600">{{ record.mileage || 'N/A' }}</td><td class="px-5 py-4 font-semibold text-slate-950">{{ money(record.bill_amount) }}</td><td class="px-5 py-4 font-semibold text-teal-700">{{ money(record.paid_amount) }}</td></tr></tbody>
                </table>
            </div>
            <div v-else class="px-5 py-14 text-center"><p class="text-lg font-semibold text-slate-950">No service rows available</p><p class="mt-2 text-sm font-semibold text-slate-500">The search summary remains available above.</p></div>
        </section>
    </AdminLayout>
</template>
...
const Status=defineComponent({props:{success:{type:Boolean,required:true}},setup(p){return()=>h('span',{class:['inline-flex rounded-md px-3 py-2 text-xs font-semibold',p.success?'bg-teal-50 text-teal-700 ring-1 ring-teal-100':'bg-red-50 text-red-700 ring-1 ring-red-100']},p.success?'Successful lookup':'Failed lookup');}});
