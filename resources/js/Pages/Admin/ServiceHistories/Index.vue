    ...
    <AdminLayout title="Service History Reports" eyebrow="Service operations">
        ...
        <section class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                <div>
                    <p class="text-xs font-black uppercase tracking-wide text-teal-700">Dealer service ledger</p>
                    <h2 class="mt-2 text-3xl font-black text-slate-950">Review workshop history lookups and service depth.</h2>
                ...
                <div class="flex flex-wrap gap-2">
                    <a :href="actions.exportExcel" class="rounded-lg bg-teal-700 px-4 py-3 text-sm font-black text-white hover:bg-teal-800">Export Excel</a>
                    <a :href="actions.exportPdf" class="rounded-lg border border-slate-200 px-4 py-3 text-sm font-black text-slate-700 hover:bg-slate-50">Export PDF</a>
                </div>
            </div>
        </section>
        ...
        <section class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <form class="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.1fr_1fr_160px_170px_170px_auto]" @submit.prevent="filter">
                ...
                <div class="flex items-end gap-2 md:col-span-2 xl:col-span-1">
                    <button class="h-12 rounded-lg bg-slate-950 px-5 text-sm font-black text-white hover:bg-teal-700">Filter</button>
                    <Link href="/admin/service-histories" class="grid h-12 place-items-center rounded-lg border border-slate-200 px-5 text-sm font-black text-slate-700 hover:bg-slate-50">Clear</Link>
                </div>
            </form>
        </section>
                ...
                <table class="w-full min-w-[1030px] text-left text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="item in searches.data" :key="item.id" class="hover:bg-slate-50">
                            <td class="px-5 py-4"><p class="font-black uppercase text-slate-950">{{ item.vehicle_number }}</p><p class="mt-1 text-xs font-black text-slate-400">#{{ item.id }}</p></td>
                            <td class="px-5 py-4"><Link v-if="item.dealer" :href="item.dealer.show_url" class="font-black text-slate-950 hover:text-teal-700">{{ item.dealer.name }}</Link><p v-else class="font-bold text-slate-400">Dealer unavailable</p><p v-if="item.dealer?.phone" class="mt-1 text-xs font-semibold text-slate-500">{{ item.dealer.phone }}</p></td>
                            <td class="px-5 py-4 font-black text-slate-950">{{ item.service_count }} services</td>
                            ...
                            <td class="px-5 py-4 font-black text-slate-950">{{ money(item.charge_amount) }}</td>
                            <td class="px-5 py-4 font-semibold text-slate-600">{{ item.created_at || 'N/A' }}</td>
                            <td class="px-5 py-4"><div class="flex justify-end gap-2"><Link :href="item.actions.show" class="rounded-lg bg-slate-950 px-3 py-2 text-xs font-black text-white hover:bg-teal-700">View</Link><a v-if="item.is_success" :href="item.actions.pdf" class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-50">PDF</a></div></td>
                        </tr>
                        <tr v-if="!searches.data.length"><td colspan="7" class="px-5 py-14 text-center"><p class="text-lg font-black text-slate-950">No service searches found</p><p class="mt-2 text-sm font-semibold text-slate-500">Try changing or clearing the filters.</p></td></tr>
                    </tbody>
                </table>
            </div>
...
const Field=defineComponent({props:{label:{type:String,required:true}},setup(p,{slots}){return()=>h('label',[h('span',{class:'mb-2 block text-sm font-black text-slate-700'},p.label),slots.default?.()]);}});
const Metric=defineComponent({props:{label:{type:String,required:true},value:{type:[String,Number],required:true},tone:{type:String,default:'slate'}},setup(p){const c:any={slate:'border-slate-200 bg-white',teal:'border-teal-100 bg-teal-50',red:'border-red-100 bg-red-50',blue:'border-blue-100 bg-blue-50',orange:'border-orange-100 bg-orange-50'};return()=>h('div',{class:['rounded-lg border p-4 shadow-sm',c[p.tone]]},[h('p',{class:'text-2xl font-black text-slate-950'},typeof p.value==='number'?new Intl.NumberFormat('en-IN').format(p.value):p.value),h('p',{class:'mt-1 text-xs font-black uppercase tracking-wide text-slate-500'},p.label)]);}});
const Status=defineComponent({props:{success:{type:Boolean,required:true}},setup(p){return()=>h('span',{class:['inline-flex rounded-md px-2.5 py-1 text-xs font-black',p.success?'bg-teal-50 text-teal-700 ring-1 ring-teal-100':'bg-red-50 text-red-700 ring-1 ring-red-100']},p.success?'Successful':'Failed');}});
</script>
