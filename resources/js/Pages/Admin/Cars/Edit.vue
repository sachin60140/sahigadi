            ...
            <div v-if="car.images.length" class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <article v-for="image in car.images" :key="image.id" class="overflow-hidden rounded-lg border border-slate-200 bg-slate-50">
                    <div class="relative aspect-[4/3] bg-slate-100">
                        ...
                        <span v-if="image.is_primary" class="absolute left-3 top-3 rounded-md bg-teal-700 px-2.5 py-1 text-xs font-black text-white">Primary</span>
                    </div>
                    <div class="grid gap-2 p-3 sm:grid-cols-2">
                        <button
                            type="button"
                            class="rounded-lg border border-teal-200 bg-teal-50 px-3 py-2 text-xs font-black text-teal-700 disabled:cursor-not-allowed disabled:opacity-50"
