    ...
    <AdminLayout title="Featured Plans" eyebrow="Revenue settings">
        ...
        <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wide text-teal-700">Listing promotion</p>
                    <h2 class="mt-2 text-3xl font-semibold text-slate-950">Price premium visibility by duration.</h2>
                ...
                <Link :href="actions.create" class="inline-flex w-fit rounded-lg bg-orange-500 px-4 py-3 text-sm font-semibold text-white transition hover:bg-orange-600">Add featured plan</Link>
            </div>
                ...
                <table class="w-full min-w-[820px] text-left text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="plan in plans" :key="plan.id" class="hover:bg-slate-50">
                            <td class="px-5 py-4 font-semibold text-slate-950">{{ plan.name }}</td>
                            <td class="px-5 py-4 font-semibold text-slate-700">{{ plan.duration_days }} days</td>
                            <td class="px-5 py-4 text-lg font-semibold text-teal-700">{{ formatCurrency(plan.price) }}</td>
                            <td class="px-5 py-4">
                                <p class="font-semibold text-slate-950">{{ plan.active_featured_listings_count }} active</p>
                        ...
                        <tr v-if="!plans.length">
                            <td colspan="6" class="px-5 py-14 text-center">
                                <p class="text-lg font-semibold text-slate-950">No featured plans configured</p>
