        ...
        <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="grid gap-5 lg:grid-cols-2">
                <Field label="Plan name" :error="form.errors.name" required>
                    <input v-model="form.name" class="admin-input" required type="text" placeholder="Growth Plan" />
                </Field>
                ...
                <Field label="Listing limit" :error="form.errors.listing_limit" required>
                    <input v-model="form.listing_limit" class="admin-input" min="1" required type="number" />
                </Field>
                ...
                <div class="lg:col-span-2">
                    <Field label="Description" :error="form.errors.description">
                        <textarea v-model="form.description" class="admin-input min-h-32" placeholder="Explain who this subscription is for and what it includes."></textarea>
                    </Field>
                </div>
                <label class="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 lg:col-span-2">
                    <input v-model="form.is_active" class="mt-1 h-5 w-5 accent-teal-700" type="checkbox" />
                    <span>
                        <span class="block text-sm font-semibold text-slate-950">Active subscription plan</span>
                        <span class="mt-1 block text-xs font-semibold leading-5 text-slate-500">Available for dealer purchase and admin assignment.</span>
                    </span>
                </label>
            </div>
        </section>
