    ...
    <AdminLayout title="Subscription Plans" eyebrow="Revenue settings">
        ...
        <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                <div>
                    <p class="text-xs font-black uppercase tracking-wide text-teal-700">Dealer subscriptions</p>
                    <h2 class="mt-2 text-3xl font-black text-slate-950">Define inventory capacity and access duration.</h2>
                ...
                <Link :href="actions.create" class="inline-flex w-fit rounded-lg bg-orange-500 px-4 py-3 text-sm font-black text-white transition hover:bg-orange-600">Add plan</Link>
            </div>
                ...
                <table class="w-full min-w-[940px] text-left text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="plan in plans" :key="plan.id" class="hover:bg-slate-50">
                            ...
                            <td class="px-5 py-4">
                                <p class="font-black text-slate-950">{{ plan.listing_limit }} listings</p>
                                <p class="mt-1 text-xs font-semibold text-slate-500">{{ plan.duration_days }} days</p>
                            </td>
                            <td class="px-5 py-4">
                                <p class="font-black text-slate-950">{{ plan.active_subscriptions_count }} active</p>
                        ...
                        <tr v-if="!plans.length">
                            <td colspan="6" class="px-5 py-14 text-center">
                                <p class="text-lg font-black text-slate-950">No subscription plans configured</p>
