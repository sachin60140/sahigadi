    ...
    <AdminLayout title="Vehicle Enquiries" eyebrow="Buyer leads">
        <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                <div>
                    <p class="text-xs font-black uppercase tracking-wide text-teal-700">Lead desk</p>
                    <h2 class="mt-2 text-3xl font-black text-slate-950">Turn buyer interest into timely conversations.</h2>
                ...
                <a :href="exportUrl" class="inline-flex w-fit rounded-lg bg-teal-700 px-4 py-3 text-sm font-black text-white transition hover:bg-teal-800">
                    Export CSV
                </a>
            </div>
        ...
        <section class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <form class="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.2fr_160px_190px_170px_170px_auto]" @submit.prevent="applyFilters">
                ...
                <div class="flex items-end gap-2 md:col-span-2 xl:col-span-1">
                    <button type="submit" class="h-12 rounded-lg bg-slate-950 px-5 text-sm font-black text-white transition hover:bg-teal-700">
                        Filter
                    </button>
                    <Link href="/admin/enquiries" class="grid h-12 place-items-center rounded-lg border border-slate-200 px-5 text-sm font-black text-slate-700 transition hover:bg-slate-50">
                        Clear
                    </Link>
                </div>
            </form>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="enquiry in enquiries.data" :key="enquiry.id" class="hover:bg-slate-50">
                            <td class="px-5 py-4">
                                ...
                                <p class="mt-1 text-xs font-black text-slate-400">#{{ enquiry.id }}</p>
                            </td>
                            <td class="px-5 py-4">
                                <p class="font-black text-slate-950">{{ enquiry.customer_name }}</p>
                            ...
                            <td class="px-5 py-4">
                                <template v-if="enquiry.vehicle">
                                    <Link :href="enquiry.vehicle.admin_url" class="block max-w-[260px] truncate font-black text-slate-950 hover:text-teal-700">
                                        {{ enquiry.vehicle.title }}
                                    </Link>
                                    <p class="mt-1 text-xs font-bold text-slate-500">{{ formatCurrency(enquiry.vehicle.price) }}</p>
                                </template>
                                <span v-else class="inline-flex rounded-md bg-red-50 px-2.5 py-1 text-xs font-black text-red-700 ring-1 ring-red-100">Vehicle deleted</span>
                            </td>
                            <td class="px-5 py-4 font-black uppercase text-slate-700">{{ enquiry.vehicle?.registration_number || '-' }}</td>
                            <td class="px-5 py-4">
                                <template v-if="enquiry.listed_by === 'dealer'">
                                    <Link v-if="enquiry.dealer" :href="enquiry.dealer.show_url" class="font-black text-slate-950 hover:text-teal-700">
                                        {{ enquiry.dealer.company_name || enquiry.dealer.name }}
                                    </Link>
                                    <p v-else class="font-black text-red-700">Dealer deleted</p>
                                    <span class="mt-2 inline-flex rounded-md bg-blue-50 px-2.5 py-1 text-xs font-black text-blue-700 ring-1 ring-blue-100">Dealer</span>
                                </template>
                                <template v-else>
                                    <p class="font-black text-slate-950">{{ enquiry.vehicle?.owner_name || 'Customer' }}</p>
                                    <span class="mt-2 inline-flex rounded-md bg-teal-50 px-2.5 py-1 text-xs font-black text-teal-700 ring-1 ring-teal-100">Customer</span>
                                </template>
                            </td>
                            <td class="px-5 py-4"><StatusBadge :status="enquiry.status" /></td>
                            <td class="px-5 py-4">
                                <div class="flex justify-end gap-2">
                                    <Link :href="enquiry.actions.show" class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-black text-slate-700 transition hover:bg-slate-50">
                                        View
                                    </Link>
                                    <button
                                    ...
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        class="rounded-lg border border-teal-200 bg-teal-50 px-3 py-2 text-xs font-black text-teal-700 transition hover:bg-white"
                                    >
                                        WhatsApp
                                    </a>
                                </div>
                        ...
                        <tr v-if="!enquiries.data.length">
                            <td colspan="7" class="px-5 py-14 text-center">
                                <p class="text-lg font-black text-slate-950">No vehicle enquiries found</p>
