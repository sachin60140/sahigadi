    ...
    <AdminLayout :title="`Enquiry #${enquiry.id}`" eyebrow="Buyer lead details">
        <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                <div>
                    <Link href="/admin/enquiries" class="inline-flex rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                        Back to enquiries
                    </Link>
                    <div class="mt-5 flex flex-wrap items-center gap-3">
                        <h2 class="text-3xl font-semibold text-slate-950">{{ enquiry.customer_name }}</h2>
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        class="rounded-lg bg-orange-500 px-4 py-3 text-sm font-semibold text-white transition hover:bg-orange-600"
                    >
                        Send WhatsApp
                    </a>
                </div>
        ...
        <section class="mt-5 grid gap-5 xl:grid-cols-[1.35fr_0.75fr]">
            <div class="grid content-start gap-5">
                <Panel title="Customer information" eyebrow="Buyer">
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <InfoItem label="Phone">
                            <a v-if="enquiry.phone_url" :href="enquiry.phone_url" class="font-semibold text-teal-700">{{ enquiry.customer_phone }}</a>
                            <span v-else>{{ enquiry.customer_phone || 'N/A' }}</span>
                        </InfoItem>
                        <InfoItem label="Email">
                            <a v-if="enquiry.email_url" :href="enquiry.email_url" class="break-all font-semibold text-teal-700">{{ enquiry.customer_email }}</a>
                    ...
                    <div v-if="enquiry.message" class="mt-5 border-t border-slate-100 pt-5">
                        <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Customer message</p>
                ...
                <Panel title="Vehicle details" eyebrow="Requested listing">
                        ...
                        <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                            <div>
                                <div class="flex flex-wrap items-center gap-2">
                                    <h3 class="text-2xl font-semibold text-slate-950">{{ enquiry.vehicle.title }}</h3>
                                ...
                                <p class="mt-4 text-2xl font-semibold text-teal-700">{{ formatCurrency(enquiry.vehicle.price) }}</p>
                                <p class="mt-2 text-sm font-bold uppercase text-slate-500">
                                    Registration: {{ enquiry.vehicle.registration_number || 'N/A' }}
                                </p>
                            </div>
                            <div class="flex flex-wrap gap-2">
                                <Link :href="enquiry.vehicle.admin_url" class="rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                                    Open in admin
                                </Link>
                                <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    class="rounded-lg bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-teal-700"
                                >
                                    View public page
                                </a>
                            </div>
                    ...
                    <div v-else class="rounded-lg border border-red-100 bg-red-50 p-4">
                        <p class="font-semibold text-red-700">The associated vehicle is no longer available.</p>
            ...
            <div class="grid content-start gap-5">
                <Panel title="Lead actions" eyebrow="Follow-up">
                    <div v-if="enquiry.status === 'contacted'" class="rounded-lg border border-teal-100 bg-teal-50 p-4 text-sm font-semibold text-teal-700">
                        This enquiry is marked as contacted.
                    </div>
                    <button
                        v-else
                        type="button"
                        class="w-full rounded-lg bg-teal-700 px-4 py-3 text-sm font-semibold text-white transition hover:bg-teal-800"
                        @click="markContacted"
                    >
                        Mark as contacted
                    </button>
                    <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        class="mt-3 flex w-full justify-center rounded-lg border border-teal-200 bg-teal-50 px-4 py-3 text-sm font-semibold text-teal-700 transition hover:bg-white"
                    >
                ...
                <Panel v-if="enquiry.listed_by === 'dealer'" title="Dealer information" eyebrow="Listing owner">
                    <template v-if="enquiry.dealer">
                        <p class="text-xl font-semibold text-slate-950">{{ enquiry.dealer.company_name || enquiry.dealer.name }}</p>
                        <p v-if="enquiry.dealer.company_name" class="mt-1 text-sm font-bold text-slate-500">{{ enquiry.dealer.name }}</p>
                        <a v-if="enquiry.dealer.phone" :href="`tel:${enquiry.dealer.phone}`" class="mt-4 block text-sm font-semibold text-teal-700">{{ enquiry.dealer.phone }}</a>
                        <a v-if="enquiry.dealer.email" :href="`mailto:${enquiry.dealer.email}`" class="mt-2 block break-all text-sm font-semibold text-teal-700">{{ enquiry.dealer.email }}</a>
                        <Link :href="enquiry.dealer.show_url" class="mt-5 inline-flex rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                            View dealer
                        </Link>
                    </template>
                    <div v-else class="rounded-lg border border-red-100 bg-red-50 p-4 text-sm font-semibold text-red-700">The associated dealer account has been deleted.</div>
                </Panel>

                <Panel v-else title="Listing owner" eyebrow="Customer sale">
                    <p class="text-xl font-semibold text-slate-950">{{ enquiry.vehicle?.owner_name || 'Customer-owned listing' }}</p>
...
const InfoItem = defineComponent({
    ...
    setup(itemProps, { slots }) {
        return () => h('div', { class: 'rounded-lg border border-slate-100 bg-slate-50 p-4' }, [
            h('p', { class: 'text-xs font-semibold uppercase tracking-wide text-slate-400' }, itemProps.label),
            h('div', { class: 'mt-2 text-sm font-semibold text-slate-950' }, slots.default ? slots.default() : itemProps.value),
        ]);
    },
});
</script>
