    ...
    <AdminLayout title="Customer RC Searches" eyebrow="RC operations">
        ...
        <section class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                <div>
                    <p class="text-xs font-black uppercase tracking-wide text-teal-700">Customer lookup ledger</p>
                    <h2 class="mt-2 text-3xl font-black text-slate-950">Monitor paid RC searches and payment outcomes.</h2>
        ...
        <section class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <form class="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.3fr_170px_180px_180px_auto]" @submit.prevent="applyFilters">
                <label>
                    <span class="mb-2 block text-sm font-black text-slate-700">Customer or registration</span>
                ...
                <div class="flex items-end gap-2 md:col-span-2 xl:col-span-1">
                    ...
                    <Link href="/admin/customer-vehicle-searches" class="grid h-12 place-items-center rounded-lg border border-slate-200 px-5 text-sm font-black text-slate-700 transition hover:bg-slate-50">Clear</Link>
                </div>
            </form>
        </section>
                ...
                <table class="w-full min-w-[1060px] text-left text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="search in searches.data" :key="search.id" class="hover:bg-slate-50">
                            <td class="px-5 py-4">
                                <p class="font-black text-slate-950">{{ search.customer_name || 'Guest customer' }}</p>
                            ...
                            <td class="px-5 py-4">
                                <p class="font-black text-slate-950">{{ formatCurrency(search.paid_amount) }}</p>
                                <span v-if="search.is_refunded" class="mt-2 inline-flex rounded-md bg-sky-50 px-2.5 py-1 text-xs font-black text-sky-700 ring-1 ring-sky-100">Refunded</span>
                                <span v-else class="mt-2 inline-flex rounded-md bg-slate-100 px-2.5 py-1 text-xs font-black text-slate-600">Retained</span>
                            </td>
                        ...
                        <tr v-if="!searches.data.length">
                            <td colspan="6" class="px-5 py-14 text-center">
                                <p class="text-lg font-black text-slate-950">No customer RC searches found</p>
