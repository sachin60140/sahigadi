    ...
    <AdminLayout :title="`Edit ${listing.title}`" eyebrow="Customer inventory">
        <section class="mb-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <p class="text-xs font-black uppercase tracking-wide text-teal-700">Edit listing</p>
        ...
        <section class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <p class="text-xs font-black uppercase tracking-wide text-teal-700">Image manager</p>
                    <h3 class="mt-1 text-xl font-black text-slate-950">Current images</h3>
                </div>
            ...
            <div v-if="listing.images.length" class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <article v-for="image in listing.images" :key="image.path" class="overflow-hidden rounded-lg border border-slate-200 bg-slate-50">
                    <div class="relative aspect-[4/3] bg-slate-100">
                        ...
                        <span v-if="form.primary_image === image.path" class="absolute left-3 top-3 rounded-md bg-teal-700 px-2.5 py-1 text-xs font-black text-white">Featured</span>
                    </div>
                    <div class="grid gap-2 p-3 sm:grid-cols-2">
                        <button type="button" class="rounded-lg border border-teal-200 bg-teal-50 px-3 py-2 text-xs font-black text-teal-700" @click="form.primary_image = image.path">
                            Featured
                        </button>
                        <button type="button" class="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-black text-red-700" @click="deleteImage(image)">
                            Delete
                        </button>
                    </div>
                </article>
