    ...
    <AdminLayout title="Customer Car Listings" eyebrow="Customer inventory">
        <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
                <div>
                    <p class="text-xs font-black uppercase tracking-wide text-teal-700">Individual sellers</p>
                    <h2 class="mt-2 text-3xl font-black text-slate-950">Manage customer-submitted car listings.</h2>
                ...
                <div class="flex flex-wrap gap-2">
                    <Link href="/admin/customer-listings/create" class="rounded-lg bg-orange-500 px-4 py-3 text-sm font-black text-white transition hover:bg-orange-600">
                        Create Listing
                    </Link>
                    <a :href="exportUrls.excel" class="rounded-lg bg-teal-700 px-4 py-3 text-sm font-black text-white transition hover:bg-teal-800">Export Excel</a>
                    <a :href="exportUrls.pdf" class="rounded-lg bg-red-600 px-4 py-3 text-sm font-black text-white transition hover:bg-red-700">Export PDF</a>
                </div>
            </div>
        ...
        <section class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <form class="grid gap-3 md:grid-cols-[170px_170px_180px_1fr_auto]" @submit.prevent="applyFilters">
                ...
                <label class="block">
                    <span class="mb-2 block text-sm font-black text-slate-700">Date</span>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <Link href="/admin/customer-listings" class="grid h-12 place-items-center rounded-lg border border-slate-200 px-5 text-sm font-black text-slate-700 transition hover:bg-slate-50">
                        Clear
                    </Link>
                </div>
            </form>
                        ...
                        <tr v-for="listing in listings.data" :key="listing.id" class="hover:bg-slate-50">
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <p class="max-w-[260px] truncate font-black text-slate-950">{{ listing.title }}</p>
                                        <p class="mt-1 text-xs font-bold text-slate-500">#{{ listing.unique_id }} / {{ listing.brand || 'No brand' }} / {{ listing.year || 'N/A' }}</p>
                                        <span v-if="listing.registration_number" class="mt-2 inline-flex rounded-md border border-slate-200 bg-white px-2.5 py-1 text-xs font-black text-slate-600">
                            ...
                            <td class="px-5 py-4">
                                <p class="font-black text-slate-950">{{ listing.owner_name || 'N/A' }}</p>
                                <p class="mt-1 text-xs font-bold text-slate-500">{{ listing.owner_phone || 'No phone' }}</p>
                            </td>
                            <td class="px-5 py-4 font-black text-slate-950">{{ formatCurrency(listing.price) }}</td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4">
                                ...
                                <a v-if="listing.map_url" :href="listing.map_url" target="_blank" rel="noreferrer" class="mt-2 inline-flex rounded-md bg-teal-50 px-2.5 py-1 text-xs font-black text-teal-700 ring-1 ring-teal-100">Map</a>
                            </td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex justify-end gap-2">
                                    <Link :href="listing.actions.show" class="rounded-lg border border-teal-200 bg-teal-50 px-3 py-2 text-xs font-black text-teal-700 transition hover:bg-white">View</Link>
                                    <Link :href="listing.actions.edit" class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-black text-slate-700 transition hover:bg-slate-50">Edit</Link>
                                    <button v-if="listing.status === 'pending'" type="button" class="rounded-lg bg-teal-700 px-3 py-2 text-xs font-black text-white" @click="approveListing(listing)">Approve</button>
                                </div>
                            </td>
                        </tr>
                        <tr v-if="!listings.data.length">
                            <td colspan="7" class="px-5 py-14 text-center">
                                <p class="text-lg font-black text-slate-950">No customer listings found</p>
...
const FeaturedState = defineComponent({
    ...
    setup(stateProps, { emit }) {
        return () => stateProps.listing.is_featured
            ? h('div', [
                h('span', { class: 'inline-flex rounded-md bg-amber-50 px-2.5 py-1 text-xs font-black text-amber-700 ring-1 ring-amber-100' }, 'Featured'),
                ...
                stateProps.listing.paid_featured_active
                    ? h('span', { class: 'mt-2 inline-flex rounded-md bg-teal-50 px-2.5 py-1 text-xs font-black text-teal-700 ring-1 ring-teal-100' }, 'User paid')
                    : h('button', { type: 'button', class: 'mt-2 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-black text-slate-700', onClick: () => emit('remove', stateProps.listing) }, 'Remove'),
            ])
            : h('button', { type: 'button', class: 'rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-black text-amber-700', onClick: () => emit('feature', stateProps.listing) }, 'Feature');
    },
});
    ...
    setup(modalProps, { emit }) {
        ...
        return () => h('div', { class: 'fixed inset-0 z-50 flex items-end justify-center bg-slate-950/60 p-3 sm:items-center sm:p-4' }, [
            ...
            h('form', {
                ...
                h('h2', { class: 'mt-1 text-2xl font-black text-slate-950' }, modalProps.listing.title),
