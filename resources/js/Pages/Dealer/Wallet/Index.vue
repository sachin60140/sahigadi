    ...
    <DealerLayout title="My Wallet" eyebrow="Payments and balance">
        <section class="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
            <div class="rounded-lg bg-slate-950 p-5 text-white shadow-sm sm:p-6">
                ...
                <button type="button" class="mt-6 rounded-lg bg-orange-500 px-5 py-3 text-sm font-black text-white hover:bg-orange-600" @click="showRecharge = true">Add money</button>
            </div>
            <div class="rounded-lg border border-teal-100 bg-teal-50 p-5">
                ...
                <h2 class="mt-2 text-2xl font-black text-slate-950">Fund service lookups and paid upgrades.</h2>
        ...
        <section class="mt-5 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-5 py-4"><p class="text-xs font-black uppercase tracking-wide text-teal-700">Ledger</p><h2 class="mt-1 text-xl font-black text-slate-950">Transaction history</h2></div>
            <div class="overflow-x-auto">
                <table class="w-full min-w-[900px] text-left text-sm">
                    <thead class="bg-slate-50 text-xs font-black uppercase tracking-wide text-slate-500"><tr><th class="px-5 py-3">Date</th><th class="px-5 py-3">Type</th><th class="px-5 py-3">Amount</th><th class="px-5 py-3">Remark</th><th class="px-5 py-3">Reference</th><th class="px-5 py-3 text-right">Receipt</th></tr></thead>
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="item in transactions.data" :key="item.id" class="hover:bg-slate-50">
                            ...
                            <td class="px-5 py-4"><span class="inline-flex rounded-md px-2.5 py-1 text-xs font-black capitalize" :class="item.type === 'credit' ? 'bg-teal-50 text-teal-700 ring-1 ring-teal-100' : 'bg-red-50 text-red-700 ring-1 ring-red-100'">{{ item.type }}</span></td>
                            ...
                            <td class="px-5 py-4 text-right"><a v-if="item.receipt_url" :href="item.receipt_url" class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-50">Download</a><span v-else class="text-slate-300">-</span></td>
                        </tr>
                        <tr v-if="!transactions.data.length"><td colspan="6" class="px-5 py-14 text-center"><p class="text-lg font-black text-slate-950">No transactions yet</p><p class="mt-2 text-sm font-semibold text-slate-500">Your wallet credits and service debits will appear here.</p></td></tr>
                    </tbody>
                </table>
            </div>
        ...
        <div v-if="showRecharge" class="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/60 p-3 sm:items-center sm:p-4">
            <div class="w-full max-w-lg rounded-lg bg-white shadow-2xl">
                ...
                <form :action="actions.checkout" method="GET" class="p-5">
                    ...
                    <label><span class="mb-2 block text-sm font-black text-slate-700">Recharge amount</span><input v-model.number="amount" class="dealer-input" type="number" name="recharge_amount" :min="minRechargeAmount" required /></label>
