<template>
    ...
    <DealerLayout title="My Inventory" eyebrow="Showroom stock">
        <section class="flex flex-col gap-4 rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
            <div><p class="text-xs font-semibold uppercase tracking-wide text-teal-700">Inventory control</p><h2 class="mt-2 text-2xl font-semibold text-slate-950">Manage listings, reviews and promotion.</h2></div>
            <Link :href="actions.create" class="inline-flex min-h-12 items-center justify-center rounded-lg bg-orange-500 px-5 text-sm font-semibold text-white hover:bg-orange-600">Add new car</Link>
        </section>
        ...
        <section class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <form class="grid gap-3 sm:grid-cols-[180px_minmax(0,1fr)_auto]" @submit.prevent="filter">
                <label><span class="mb-2 block text-sm font-semibold text-slate-700">Status</span><select v-model="form.status" class="dealer-input"><option value="all">All statuses</option><option value="pending">Pending</option><option value="approved">Approved</option><option value="rejected">Rejected</option></select></label>
                <label><span class="mb-2 block text-sm font-semibold text-slate-700">Search</span><input v-model="form.search" class="dealer-input" type="search" placeholder="Search listing title" /></label>
                <div class="flex items-end gap-2"><button class="h-12 rounded-lg bg-slate-950 px-5 text-sm font-semibold text-white hover:bg-teal-700">Filter</button><Link href="/dealer/cars" class="grid h-12 place-items-center rounded-lg border border-slate-200 px-5 text-sm font-semibold text-slate-700">Clear</Link></div>
            </form>
        </section>
        <section class="mt-5 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            <div class="overflow-x-auto"><table class="w-full min-w-[1060px] text-left text-sm"><thead class="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500"><tr><th class="px-5 py-3">Listing</th><th class="px-5 py-3">Price</th><th class="px-5 py-3">Review status</th><th class="px-5 py-3">Promotion</th><th class="px-5 py-3 text-right">Actions</th></tr></thead><tbody class="divide-y divide-slate-100">
                <tr v-for="car in cars.data" :key="car.id" class="hover:bg-slate-50">
                    <td class="px-5 py-4"><div class="flex items-center gap-3"><img v-if="car.image_url" :src="car.image_url" :alt="car.title" class="h-16 w-24 rounded-lg object-cover" /><div v-else class="grid h-16 w-24 place-items-center rounded-lg bg-slate-100 text-xs font-semibold text-slate-400">NO IMAGE</div><div><Link :href="car.actions.edit" class="font-semibold text-slate-950 hover:text-teal-700">{{ car.title }}</Link><p class="mt-1 text-xs font-bold text-slate-400">{{ car.unique_id }}</p><p v-if="car.rejection_reason" class="mt-1 max-w-xs text-xs font-bold text-red-600">{{ car.rejection_reason }}</p></div></div></td>
                    <td class="px-5 py-4 font-semibold text-slate-950">{{ money(car.price) }}</td>
                    <td class="px-5 py-4"><Status :value="car.status" /></td>
                    <td class="px-5 py-4"><span v-if="car.is_featured" class="inline-flex rounded-md bg-orange-50 px-2.5 py-1 text-xs font-semibold text-orange-700 ring-1 ring-orange-100">Featured</span><span v-else class="font-semibold text-slate-400">Standard</span><p v-if="car.featured_expires_at" class="mt-1 text-xs font-bold text-slate-500">Until {{ car.featured_expires_at }}</p></td>
                    <td class="px-5 py-4"><div class="flex justify-end gap-2"><Link v-if="car.status === 'approved'" :href="car.actions.feature" class="rounded-lg border border-orange-200 px-3 py-2 text-xs font-semibold text-orange-700 hover:bg-orange-50">{{ car.is_featured ? 'Extend' : 'Feature' }}</Link><Link :href="car.actions.edit" class="rounded-lg bg-slate-950 px-3 py-2 text-xs font-semibold text-white hover:bg-teal-700">Edit</Link><button type="button" class="rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-700 hover:bg-red-50" @click="remove(car)">Delete</button></div></td>
                </tr>
                <tr v-if="!cars.data.length"><td colspan="5" class="px-5 py-14 text-center"><p class="text-lg font-semibold text-slate-950">No cars found</p><Link :href="actions.create" class="mt-2 inline-flex text-sm font-semibold text-teal-700">Add your first car</Link></td></tr>
