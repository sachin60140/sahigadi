        ...
        <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            ...
            <div class="px-5 pb-5 sm:px-6">
                <div class="-mt-12 flex flex-col gap-4 sm:-mt-14 sm:flex-row sm:items-end">
                    ...
                    <div class="min-w-0 pb-1"><h2 class="truncate text-2xl font-black text-slate-950">{{ dealer.name }}</h2><p class="mt-1 text-sm font-bold text-slate-500">{{ dealer.company_name || 'Independent dealer' }} / {{ dealer.dealer_unique_id }}</p></div>
                </div>
            </div>
        </section>

        <div class="mt-5 flex overflow-x-auto" role="tablist" aria-label="Profile sections">
            <button v-for="item in tabs" :key="item.value" type="button" class="min-w-max border-b-2 px-4 py-3 text-sm font-black" :class="tab===item.value?'border-teal-700 text-teal-700':'border-transparent text-slate-500'" @click="tab=item.value">{{ item.label }}</button>
        </div>

        <section v-if="tab==='profile'" class="mt-5 grid gap-5 xl:grid-cols-[300px_minmax(0,1fr)]">
            <aside class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between gap-3"><p class="text-sm font-black text-slate-950">Profile completion</p><span class="rounded-md bg-teal-50 px-2.5 py-1 text-sm font-black text-teal-700">{{ dealer.completion }}%</span></div>
                ...
                <div v-if="dealer.missing_fields.length" class="mt-5"><p class="text-xs font-black uppercase tracking-wide text-orange-700">Still needed</p><div class="mt-3 flex flex-wrap gap-2"><span v-for="field in dealer.missing_fields" :key="field" class="rounded-md bg-orange-50 px-2.5 py-1 text-xs font-bold text-orange-700">{{ field }}</span></div></div>
            </aside>

            <form class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:p-6" @submit.prevent="saveProfile">
                <p class="text-xs font-black uppercase tracking-wide text-teal-700">Business identity</p>
                <div class="mt-5 grid gap-4 md:grid-cols-2">
                    ...
                    <Field label="Profile photo" :error="profileForm.errors.profile_image"><input class="dealer-input file:mr-3 file:rounded-md file:border-0 file:bg-slate-100 file:px-3 file:py-2 file:text-xs file:font-black" type="file" accept="image/*" @change="selectProfileImage" /></Field>
                    ...
                    <Field label="PAN document" :error="profileForm.errors.pan_document_path"><input class="dealer-input" type="file" accept=".jpg,.jpeg,.png,.pdf" @change="profileForm.pan_document_path=fileFrom($event)" /><a v-if="dealer.pan_document_url" :href="dealer.pan_document_url" target="_blank" rel="noopener noreferrer" class="mt-2 inline-flex text-xs font-black text-teal-700">View current document</a></Field>
                    ...
                    <Field label="KYC document" :error="profileForm.errors.kyc_document_path"><input class="dealer-input" type="file" accept=".jpg,.jpeg,.png,.pdf" @change="profileForm.kyc_document_path=fileFrom($event)" /><a v-if="dealer.kyc_document_url" :href="dealer.kyc_document_url" target="_blank" rel="noopener noreferrer" class="mt-2 inline-flex text-xs font-black text-teal-700">View current document</a></Field>
                </div>
                <p class="mt-8 text-xs font-black uppercase tracking-wide text-teal-700">Location</p>
                ...
                <button class="mt-6 rounded-lg bg-teal-700 px-5 py-3 text-sm font-black text-white hover:bg-teal-800 disabled:opacity-60" :disabled="profileForm.processing">{{ profileForm.processing?'Saving...':'Save profile' }}</button>
            </form>
        </section>

        <section v-else class="mt-5 grid gap-5 xl:grid-cols-2">
            <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <p class="text-xs font-black uppercase tracking-wide text-teal-700">Mobile verification</p><h2 class="mt-2 text-xl font-black text-slate-950">Update registered phone</h2><p class="mt-2 text-sm font-semibold text-slate-600">Current number: {{ dealer.phone }}</p>
                <div class="mt-5 flex flex-col gap-2 sm:flex-row"><input v-model="newPhone" class="dealer-input" maxlength="10" inputmode="numeric" placeholder="New 10-digit number" :readonly="otpSent" /><button v-if="!otpSent" type="button" class="min-h-12 shrink-0 rounded-lg bg-slate-950 px-5 text-sm font-black text-white disabled:opacity-50" :disabled="sendingOtp" @click="sendOtp">{{ sendingOtp?'Sending...':'Request OTP' }}</button></div>
                ...
                <form v-if="otpSent" class="mt-5 grid gap-4 sm:grid-cols-2" @submit.prevent="verifyPhone">
                    ...
                    <button class="rounded-lg bg-teal-700 px-5 py-3 text-sm font-black text-white sm:col-span-2" :disabled="phoneForm.processing">Verify and update phone</button>
                </form>
            </div>

            <form class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm" @submit.prevent="updatePassword">
                <p class="text-xs font-black uppercase tracking-wide text-teal-700">Account security</p><h2 class="mt-2 text-xl font-black text-slate-950">Change password</h2>
                ...
                <button class="mt-5 rounded-lg bg-slate-950 px-5 py-3 text-sm font-black text-white hover:bg-teal-700 disabled:opacity-60" :disabled="passwordForm.processing">Update password</button>
            </form>
        </section>
    </DealerLayout>
</template>
