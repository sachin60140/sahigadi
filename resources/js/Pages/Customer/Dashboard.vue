    ...
    <CustomerLayout title="My Dashboard" eyebrow="Listings and account">
        ...
        <section class="mt-6">
            <div class="flex flex-wrap items-end justify-between gap-4">
                <div>
                    <p class="text-xs font-black uppercase tracking-wide text-teal-700">Your inventory</p>
                    <h2 class="mt-1 text-2xl font-black text-slate-950">Car listings</h2>
                </div>
                <Link :href="actions.sellCar" class="inline-flex items-center gap-2 rounded-lg bg-orange-500 px-4 py-3 text-sm font-black text-white hover:bg-orange-600">
                    <Plus class="h-4 w-4" />
                    Sell a car
                </Link>
            </div>
                ...
                <article v-for="listing in listings" :key="listing.id" class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
                    <div class="relative aspect-[16/10] bg-slate-100">
                        ...
                        <div class="absolute left-3 top-3 flex flex-wrap gap-2">
                            <span class="rounded-md px-2.5 py-1 text-xs font-black capitalize" :class="statusClass(listing.status)">
                                {{ listing.status === 'pending' ? 'Pending review' : listing.status }}
                            </span>
                            <span v-if="listing.is_featured" class="inline-flex items-center gap-1 rounded-md bg-amber-100 px-2.5 py-1 text-xs font-black text-amber-800">
                    ...
                    <div class="p-5">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <h3 class="truncate text-lg font-black text-slate-950">{{ listing.title }}</h3>
                                <p class="mt-1 text-xs font-bold text-slate-500">{{ listing.unique_id }}</p>
                            </div>
                            <p class="shrink-0 text-lg font-black text-teal-700">{{ money(listing.price) }}</p>
                        </div>
                        ...
                        <p v-if="listing.is_featured && listing.featured_expires_at" class="mt-3 text-xs font-black text-amber-700">
                    ...
                    <div class="grid grid-cols-2 gap-2 border-t border-slate-100 p-4">
                        <a
                            ...
                            class="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 px-3 py-2.5 text-sm font-black text-slate-700 hover:bg-slate-50"
                        >
                        ...
                        <Link
                            ...
                            class="inline-flex items-center justify-center gap-2 rounded-lg bg-amber-100 px-3 py-2.5 text-sm font-black text-amber-800 hover:bg-amber-200"
                        >
                        ...
                        <Link
                            ...
                            class="col-span-2 inline-flex items-center justify-center gap-2 rounded-lg border border-teal-200 bg-teal-50 px-3 py-2.5 text-sm font-black text-teal-700 hover:bg-teal-100"
                        >
                            <Pencil class="h-4 w-4" /> Edit listing
                        </Link>
                        <button
                            type="button"
                            class="col-span-2 inline-flex items-center justify-center gap-2 rounded-lg border border-red-200 px-3 py-2.5 text-sm font-black text-red-700 hover:bg-red-50"
            ...
            <div v-else class="mt-5 rounded-lg border border-dashed border-slate-300 bg-white px-5 py-14 text-center">
                ...
                <h3 class="mt-4 text-xl font-black text-slate-950">No cars listed yet</h3>
                <p class="mt-2 text-sm font-semibold text-slate-500">Create your first listing to start receiving buyer enquiries.</p>
                <Link :href="actions.sellCar" class="mt-5 inline-flex rounded-lg bg-teal-700 px-5 py-3 text-sm font-black text-white hover:bg-teal-800">
                    Sell your first car
                </Link>
            </div>
        </section>
        ...
        <div v-if="deleteListing" class="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/60 p-3 sm:items-center sm:p-4">
            <div class="w-full max-w-md rounded-lg bg-white shadow-2xl">
                <div class="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
                    <div>
                        <p class="text-xs font-black uppercase tracking-wide text-red-600">Security verification</p>
                        <h2 class="mt-1 text-xl font-black text-slate-950">Delete listing</h2>
                    </div>
                ...
                <div class="p-5">
                    ...
                    <div v-if="deleteStep === 'send'" class="mt-5">
                        ...
                        <button type="button" class="mt-4 w-full rounded-lg bg-slate-950 px-4 py-3 text-sm font-black text-white disabled:opacity-50" :disabled="deleteBusy" @click="sendDeleteOtp">
                    ...
                    <div v-else class="mt-5">
                        <label>
                            <span class="mb-2 block text-sm font-black text-slate-700">6-digit OTP</span>
                        ...
                        <button type="button" class="mt-4 w-full rounded-lg bg-red-600 px-4 py-3 text-sm font-black text-white disabled:opacity-50" :disabled="deleteBusy || deleteOtp.length !== 6" @click="confirmDelete">
                            {{ deleteBusy ? 'Verifying...' : 'Verify and delete' }}
                        </button>
                        <button type="button" class="mt-3 w-full text-sm font-black text-slate-500" :disabled="deleteBusy" @click="sendDeleteOtp">Resend OTP</button>
                    </div>
...
const StatCard = defineComponent({
    ...
    setup(p) {
        ...
        return () => h('article', { class: 'rounded-lg border border-slate-200 bg-white p-5 shadow-sm' }, [
            ...
            h('p', { class: 'mt-4 text-3xl font-black text-slate-950' }, String(p.value)),
