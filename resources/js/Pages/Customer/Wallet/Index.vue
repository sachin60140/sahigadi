    ...
    <CustomerLayout title="My Wallet" eyebrow="Balance and transactions">
        <section class="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
            <div class="rounded-lg bg-slate-950 p-5 text-white shadow-sm sm:p-6">
                ...
                <button type="button" class="mt-6 inline-flex items-center gap-2 rounded-lg bg-orange-500 px-5 py-3 text-sm font-semibold text-white hover:bg-orange-600" @click="showRecharge = true">
                    <Plus class="h-4 w-4" />
                    Add money
                </button>
            </div>
            <div class="rounded-lg border border-teal-100 bg-teal-50 p-5 sm:p-6">
                ...
                <h2 class="mt-2 text-2xl font-semibold text-slate-950">Pay for verification and listing upgrades.</h2>
        ...
        <section class="mt-5 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-5 py-4">
                <p class="text-xs font-semibold uppercase tracking-wide text-teal-700">Ledger</p>
                <h2 class="mt-1 text-xl font-semibold text-slate-950">Transaction history</h2>
            </div>
                ...
                <table class="w-full min-w-[820px] text-left text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-for="item in transactions.data" :key="item.id" class="hover:bg-slate-50">
                            ...
                            <td class="px-5 py-4"><span class="rounded-md px-2.5 py-1 text-xs font-semibold capitalize" :class="item.type === 'credit' ? 'bg-teal-50 text-teal-700 ring-1 ring-teal-100' : 'bg-red-50 text-red-700 ring-1 ring-red-100'">{{ item.type }}</span></td>
                            ...
                            <td class="px-5 py-4 text-right"><a v-if="item.receipt_url" :href="item.receipt_url" class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50"><Download class="h-3.5 w-3.5" /> Receipt</a><span v-else class="text-slate-300">-</span></td>
                        </tr>
                        <tr v-if="!transactions.data.length"><td colspan="6" class="px-5 py-14 text-center"><WalletCards class="mx-auto h-10 w-10 text-slate-300" /><p class="mt-3 text-lg font-semibold text-slate-950">No transactions yet</p></td></tr>
                    </tbody>
                </table>
            </div>
        ...
        <div v-if="showRecharge" class="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/60 p-3 sm:items-center sm:p-4">
            <div class="w-full max-w-lg rounded-lg bg-white shadow-2xl">
                ...
                <form :action="actions.checkout" method="GET" class="p-5">
                    <label><span class="mb-2 block text-sm font-semibold text-slate-700">Recharge amount</span><input v-model.number="amount" class="customer-input" type="number" name="recharge_amount" :min="minRechargeAmount" required /></label>
