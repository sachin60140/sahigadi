    ...
    <CustomerLayout title="Promote Listing" eyebrow="Featured inventory">
        <section class="grid gap-5 xl:grid-cols-[1fr_280px]">
            <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <Link :href="actions.dashboard" class="inline-flex items-center gap-2 text-sm font-semibold text-teal-700"><ArrowLeft class="h-4 w-4" /> Back to dashboard</Link>
                <h2 class="mt-4 text-3xl font-semibold text-slate-950">{{ listing.title }}</h2>
            ...
            <div class="rounded-lg bg-slate-950 p-5 text-white">
                <p class="text-xs font-semibold uppercase text-teal-300">Wallet balance</p>
                <p class="mt-2 text-3xl font-semibold">{{ money(walletBalance) }}</p>
                <Link :href="actions.wallet" class="mt-4 inline-flex text-sm font-semibold text-orange-300">Recharge wallet</Link>
            </div>
        </section>
        ...
        <section class="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <article v-for="(plan, index) in plans" :key="plan.id" class="rounded-lg border bg-white p-5 shadow-sm" :class="index === 0 ? 'border-orange-200' : 'border-slate-200'">
                <span v-if="index === 0" class="inline-flex rounded-md bg-orange-50 px-2.5 py-1 text-xs font-semibold text-orange-700">Popular</span>
                <h3 class="mt-3 text-xl font-semibold text-slate-950">{{ plan.name }}</h3>
                <p class="mt-2 text-3xl font-semibold text-teal-700">{{ money(plan.price) }}</p>
                ...
                <button type="button" class="mt-6 w-full rounded-lg bg-slate-950 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50" :disabled="processing === plan.id" @click="purchase(plan)">{{ processing === plan.id ? 'Processing...' : 'Buy with wallet' }}</button>
            </article>
            <div v-if="!plans.length" class="rounded-lg border border-slate-200 bg-white p-8 text-center md:col-span-2 xl:col-span-3"><p class="font-semibold text-slate-950">No featured plans available</p></div>
        </section>
    </CustomerLayout>
</template>
