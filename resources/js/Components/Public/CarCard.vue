    ...
    >
        <div class="relative aspect-[16/10] overflow-hidden bg-slate-100">
            ...
            <div class="absolute left-3 top-3 flex flex-wrap gap-2">
                <span v-if="isVerified" class="rounded-md bg-teal-700 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-white shadow-sm">
                    Verified
                </span>
                <span v-if="car.is_featured" class="rounded-md bg-orange-50 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-orange-700 ring-1 ring-orange-200">
                    Featured
                </span>
            </div>

            <span class="absolute right-3 top-3 rounded-md bg-slate-950/85 px-2.5 py-1 text-xs font-semibold text-white">
                {{ car.year || 'N/A' }}
            </span>
        </div>
        ...
        <div class="flex flex-1 flex-col p-4">
            <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                    <h3 class="line-clamp-2 text-base font-semibold leading-snug text-slate-950 transition group-hover:text-teal-700">
                        {{ title }}
                    </h3>
            ...
            <div class="mt-5 flex flex-1 flex-col items-start gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-xl font-semibold text-slate-950">₹{{ formattedPrice }}</p>
                </div>
                <span class="inline-flex rounded-lg bg-slate-950 px-3 py-2 text-sm font-semibold text-white transition group-hover:bg-teal-700">
                    Details
                </span>
            </div>
        </div>
