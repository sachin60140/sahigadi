<template>
    <div v-if="profile" class="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex items-start gap-4">
            <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-teal-50 text-lg font-black text-teal-700">
                {{ sellerInitial }}
            </div>
            <div class="min-w-0 flex-1">
                <div class="flex flex-wrap items-center gap-2">
                    <p class="truncate text-lg font-black text-slate-950">{{ profile.display_name }}</p>
                    <span v-if="profile.is_verified" class="inline-flex items-center rounded-md bg-teal-50 px-2 py-1 text-xs font-black text-teal-700 ring-1 ring-teal-100">
