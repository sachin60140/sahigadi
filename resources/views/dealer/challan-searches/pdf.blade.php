...
<body>
    ...
    <div class="header">
        ...
        <div class="vehicle-box">
            ...
            <div class="number">{{ $challanSearch->vehicle_number }}</div>
        </div>
    </div>

    <div class="stats">
        <div class="stat danger">
            <h3>Rs.{{ number_format($unpaidAmount + $pendingAmount) }}</h3>
            <p>Pending Amount</p>
        </div>
        <div class="stat success">
            <h3>Rs.{{ number_format($paidAmount) }}</h3>
            <p>Paid Amount</p>
        </div>
        <div class="stat warning">
            <h3>Rs.{{ number_format($unpaidAmount) }}</h3>
            <p>Unpaid Amount</p>
        </div>
        <div class="stat primary">
            <h3>{{ count($challans) }}</h3>
            <p>Total Challans</p>
        </div>
    </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $c['nameRTO'] ?? 'N/A' }}</td>
                    <td class="{{ $isPaid ? 'amount-paid' : ($isPending ? 'amount-pending' : 'amount-unpaid') }}">Rs.{{ number_format($c['amountChallan'] ?? 0) }}</td>
                    <td>
                        @if($isPaid)
    ...
    <div class="section">
        ...
        <table class="summary">
            <tr><td class="label">Total Challans</td><td class="value">{{ count($challans) }}</td></tr>
            <tr><td class="label">Total Amount</td><td class="value">Rs.{{ number_format($challanSearch->total_amount ?? 0) }}</td></tr>
            <tr><td class="label">Paid Amount</td><td class="value amount-paid">Rs.{{ number_format($paidAmount) }}</td></tr>
            <tr><td class="label">Unpaid Amount</td><td class="value amount-unpaid">Rs.{{ number_format($unpaidAmount) }}</td></tr>
            <tr><td class="label">Pending Amount</td><td class="value amount-pending">Rs.{{ number_format($pendingAmount) }}</td></tr>
            <tr><td class="label">Physical Court</td><td class="value">{{ $physicalCourt }}</td></tr>
            <tr><td class="label">Virtual Court</td><td class="value">{{ $virtualCourt }}</td></tr>
            <tr><td class="label">Search Date</td><td class="value">{{ $challanSearch->created_at->setTimezone('Asia/Kolkata')->format('d M Y, h:i A') }}</td></tr>
            <tr><td class="label">Service Charge</td><td class="value">Rs.{{ number_format($challanSearch->charge_amount, 2) }}</td></tr>
        </table>
    </div>
    @else
    <div class="section">
        ...
        <p style="padding: 20px; text-align: center; color: #666;">
            {{ $challanSearch->error_message ?? 'No challans found for this vehicle' }}
        </p>
    </div>
    @endif
