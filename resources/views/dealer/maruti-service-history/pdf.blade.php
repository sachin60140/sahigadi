...
<body>
    ...
    <div class="header">
        ...
        <span class="badge badge-success">{{ $marutiServiceHistory->records->count() }} Services Found</span>
    </div>
    ...
    <div class="section">
        ...
        <table>
            <tr><td><strong>Total Services</strong></td><td>{{ $marutiServiceHistory->records->count() }}</td></tr>
            <tr><td><strong>Search Date</strong></td><td>{{ $marutiServiceHistory->created_at->setTimezone('Asia/Kolkata')->format('d M Y, h:i A') }}</td></tr>
            <tr><td><strong>Amount Charged</strong></td><td>Rs.{{ number_format($marutiServiceHistory->debit_amount ?? 0, 2) }}</td></tr>
        </table>
    </div>
    @else
    <div class="section">
        <p><strong>Error:</strong> {{ $marutiServiceHistory->error_message ?? 'Search failed' }}</p>
    </div>
    @endif
