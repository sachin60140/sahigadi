...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-1">
                <li class="breadcrumb-item"><a href="{{ route('dealer.maruti-service-history.index') }}">Maruti Service History</a></li>
                <li class="breadcrumb-item active">{{ $marutiServiceHistory->vehicle_number }}</li>
            </ol>
        </nav>
        <h2><i class="bi bi-car-front me-2"></i>{{ $marutiServiceHistory->vehicle_number }}</h2>
    </div>
    <div class="d-flex gap-2">
        @if($marutiServiceHistory->is_success)
            ...
            <a href="{{ route('dealer.maruti-service-history.pdf', $marutiServiceHistory) }}" class="btn btn-danger">
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-info">{{ $record->service_cate ?? 'N/A' }}</span>
                                </td>
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body">
                <div class="detail-item mb-3">
                    ...
                    <h6 class="mb-0">{{ $marutiServiceHistory->created_at->format('d M Y, h:i A') }}</h6>
                </div>
                <div class="detail-item mb-3">
                    <small class="text-muted">Total Services</small>
                    <h6 class="mb-0">{{ $marutiServiceHistory->records->count() }}</h6>
                </div>
                <div class="detail-item mb-3">
                    <small class="text-muted">Amount Charged</small>
                    <h5 class="mb-0 text-success">₹{{ number_format($marutiServiceHistory->debit_amount ?? 0, 2) }}</h5>
                </div>
                <hr>
                <a href="{{ route('dealer.maruti-service-history.index') }}" class="btn btn-outline-primary w-100">
                    <i class="bi bi-arrow-left me-2"></i>Back to Search
                </a>
                <a href="{{ route('dealer.maruti-service-history.search') }}?vehicle_number={{ $marutiServiceHistory->vehicle_number }}" class="btn btn-primary w-100 mt-2">
