        ...
        <table class="profile" style="table-layout: fixed;">
            <tr>
                ...
                <td style="width: 50%;">
                    ...
                    <div class="field-value">{{ $dealer->name ?: 'Dealer' }}</div>
                    <div style="margin-top: 4px; color: #5a6b82; font-size: 7.5px; line-height: 1.6;">
                        {{ $dealer->dealer_unique_id ? 'ID '.$dealer->dealer_unique_id : '' }}
                        @if($dealer->company_name ?? $dealer->firm_name ?? null) <br>{{ $dealer->company_name ?? $dealer->firm_name }} @endif
                        <br>{{ collect([$dealer->address, $dealer->city, $dealer->state, $dealer->pincode])->filter()->implode(', ') ?: 'Address not provided' }}
                        <br>{{ $dealer->phone }}{{ $dealer->email ? ' | '.$dealer->email : '' }}
                        @if($dealer->gst_number) <br>GST {{ $dealer->gst_number }} @endif
                    </div>
                </td>
            </tr>
        </table>
