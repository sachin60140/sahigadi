...
@section('title', 'Mahindra Service History Result - SAHI GADI')
        ...
        <div class="col-lg-10">
                    ...
                    <div class="card-body">
                        @if(isset($cached) && $cached)
                            <div class="alert alert-warning d-flex justify-content-between align-items-center flex-wrap gap-2">
                                ...
                                <form action="{{ route('mahindra-service-history.search') }}" method="POST" class="m-0">
                                    @csrf
                                    <input type="hidden" name="vehicle_number" value="{{ $mahindraServiceHistory->vehicle_number }}">
                                    <input type="hidden" name="customer_name" value="{{ $mahindraServiceHistory->customer_name ?? '' }}">
                                    <input type="hidden" name="customer_phone" value="{{ $mahindraServiceHistory->customer_phone ?? '' }}">
                                    <input type="hidden" name="customer_email" value="{{ $mahindraServiceHistory->customer_email ?? '' }}">
                        ...
                        <div class="mb-4">
                            <h5 class="fw-bold">Vehicle: {{ $mahindraServiceHistory->vehicle_number }}</h5>
                            @if($mahindraServiceHistory->customer_name)
                                <p class="text-muted mb-0">Requested by: {{ $mahindraServiceHistory->customer_name }}</p>
                            @endif
                        </div>
                        ...
                        <div class="mt-4 text-center">
                            <a href="{{ route('mahindra-service-history.pdf', $mahindraServiceHistory->id) }}" class="btn btn-primary">
            ...
            @else
                <div class="card border-0 shadow-lg">
                    ...
                    <div class="card-body">
                        <p class="mb-0">{{ $mahindraServiceHistory->error_message ?? 'No service history records found for this vehicle.' }}</p>
                        <p class="text-muted mt-3">This could mean:</p>
                        <ul>
                            <li>Vehicle is not registered with mahindra authorized service centers</li>
