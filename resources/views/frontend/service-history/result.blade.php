...
@section('title', 'Service History Result - SAHI GADI')
                    ...
                    <div class="card-body">
                        @if(isset($cached) && $cached)
                            <div class="alert alert-warning d-flex justify-content-between align-items-center flex-wrap gap-2">
                                ...
                                <form action="{{ route('service-history.search') }}" method="POST" class="m-0">
                                    @csrf
                                    <input type="hidden" name="vehicle_number" value="{{ $serviceHistory->vehicle_number }}">
                                    <input type="hidden" name="customer_name" value="{{ $serviceHistory->customer_name ?? '' }}">
                                    <input type="hidden" name="customer_phone" value="{{ $serviceHistory->customer_phone ?? '' }}">
                                    <input type="hidden" name="customer_email" value="{{ $serviceHistory->customer_email ?? '' }}">
                        ...
                        <div class="mt-4 text-center">
                            <a href="{{ route('service-history.download-pdf', $serviceHistory->id) }}" class="btn btn-primary">
