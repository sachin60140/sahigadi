            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-body p-4 text-center">
                    @if(Auth::guard('customer')->user()->profile_image)
                        <img src="{{ asset('storage/' . Auth::guard('customer')->user()->profile_image) }}" alt="Profile" class="rounded-circle mb-3 shadow-sm border border-2 border-white" style="width: 80px; height: 80px; object-fit: cover;">
                    @else
                    ...
                    <div class="bg-success bg-opacity-10 rounded-3 p-3 mt-2">
                        ...
                        <h4 class="text-success fw-bold mb-0">₹{{ number_format($walletBalance, 2) }}</h4>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body p-3">
                    <ul class="nav flex-column nav-pills custom-sidebar-nav">
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('customer.dashboard') ? 'active bg-primary text-white' : 'text-dark' }} rounded-3 px-3 py-2" href="{{ route('customer.dashboard') }}">
                                <i class="bi bi-grid-1x2 me-2"></i> Dashboard
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('customer.profile.edit') ? 'active bg-primary text-white' : 'text-dark' }} rounded-3 px-3 py-2" href="{{ route('customer.profile.edit') }}">
                                <i class="bi bi-person me-2"></i> My Profile
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('customer.enquiries.*') ? 'active bg-primary text-white' : 'text-dark' }} rounded-3 px-3 py-2" href="{{ route('customer.enquiries.index') }}">
                                <i class="bi bi-chat-left-dots me-2"></i> Enquiries
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('customer.wallet.index') ? 'active bg-primary text-white' : 'text-dark' }} rounded-3 px-3 py-2" href="{{ route('customer.wallet.index') }}">
                                <i class="bi bi-wallet2 me-2"></i> My Wallet
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link text-dark rounded-3 px-3 py-2" href="{{ route('sell-car.index') }}">
                                <i class="bi bi-car-front me-2"></i> Sell a Car
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('vehicle-search.*') ? 'active bg-primary text-white' : 'text-dark' }} rounded-3 px-3 py-2" href="{{ route('vehicle-search.index') }}">
                                <i class="bi bi-search me-2"></i> RC Search
                            </a>
                        </li>
                        <li class="nav-item mb-2">
                            <a class="nav-link {{ request()->routeIs('mahindra-service-history.*') ? 'active bg-primary text-white' : 'text-dark' }} rounded-3 px-3 py-2" href="{{ route('mahindra-service-history.index') }}">
