...
<div class="row">
    ...
    <div class="col-md-6">
        <div class="card">
            ...
            <div class="card-body">
                @if($isRazorpayActive)
                <form id="razorpay-form" action="{{ route('customer.payments.success') }}" method="POST" class="mb-3">
                    @csrf
                ...
                @if($isPhonePeActive)
                <form id="phonepe-form" action="{{ route('customer.payments.phonepe.initiate') }}" method="POST">
                    @csrf
                    <input type="hidden" name="amount" value="{{ $amount }}">
                    <input type="hidden" name="type" value="{{ $type }}">

                    <button type="submit" class="btn btn-success btn-lg w-100">
                        <i class="bi bi-phone"></i> Pay ₹{{ number_format($amount, 2) }} with PhonePe
                    </button>
                </form>
                @endif
            </div>
