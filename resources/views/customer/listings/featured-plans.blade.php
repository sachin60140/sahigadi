...
@section('title', 'Promote Your Listing')
        ...
        <div class="col-lg-10">
            ...
            <div class="d-flex align-items-center justify-content-between mb-4">
                <div class="d-flex align-items-center">
                    <a href="{{ route('customer.dashboard') }}" class="btn btn-light btn-sm me-3 border shadow-sm">
                        <i class="bi bi-arrow-left"></i>
                    </a>
                    <div>
                        <h3 class="mb-0 text-gray-800">Promote "{{ $customerListing->model }}"</h3>
                ...
                <div class="text-end d-none d-md-block">
                    ...
                    <h4 class="mb-0 text-primary">₹{{ number_format(auth('customer')->user()->wallet->balance ?? 0, 2) }}</h4>
                </div>
            </div>

            <!-- Current Status -->
            @if($customerListing->isFeatured())
            <div class="alert alert-warning border-0 shadow-sm d-flex align-items-center">
                <i class="bi bi-star-fill fs-3 me-3 text-warning"></i>
                <div>
                    <h5 class="alert-heading mb-1">Currently Featured</h5>
                    <p class="mb-0">This car is featured until <strong>{{ $customerListing->featured_expires_at->format('d M Y, h:i A') }}</strong>.</p>
                ...
                <div class="col-md-4">
                    <div class="card h-100 border-0 shadow-sm position-relative feature-plan-card" style="transition: transform 0.3s ease;">
                        ...
                        <div class="card-body text-center p-5">
                            <h4 class="text-primary fw-bold mb-3">{{ $plan->name }}</h4>
                            <h1 class="display-5 fw-bolder mb-0 text-dark">₹{{ number_format($plan->price) }}</h1>
                            <p class="text-muted mb-4">for {{ $plan->duration_days }} Days</p>
                            ...
                            <form action="{{ route('customer.listing.featured-purchase', $customerListing) }}" method="POST" class="purchase-form no-loader">
                                @csrf
                                <input type="hidden" name="plan_id" value="{{ $plan->id }}">
                                <button type="submit" class="btn btn-warning w-100 fw-bold py-2 btn-purchase" data-price="{{ $plan->price }}">
                                    Buy Now
                                </button>
                            </form>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const walletBalance = {{ auth('customer')->user()->wallet->balance ?? 0 }};
            ...
            if (price > walletBalance) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        window.location.href = "{{ route('customer.wallet.index') }}";
                    }
                });
                return;
            }
